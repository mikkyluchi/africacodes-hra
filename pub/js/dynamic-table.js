var Script = function () {
        ...
        jQuery('#sample_1 .group-checkable').change(function () {
            ...
            jQuery(set).each(function () {
                if (checked) {
					$(".showOnlyWhenAllCheckboxSelected").show();
                    $(this).attr("checked", true);
                } else {
					$(".showOnlyWhenAllCheckboxSelected").hide();
		...
		jQuery('#sample_1 .group-checkable-single').live("change", function() {
			//Get the number of checked checkboxes
			var count = $(document).find('input[name="NURSERY_GROUPS_CATEGORY_ID[]"]:checked').length;
			//check if more than one is checked
			switch(count){
				
				case 0:
					$(".showOnlyWhenAllCheckboxSelected").hide();
					$(".showOnlyWhenOneCheckboxSelected").hide();
				break;	
				case 1:
					$(".showOnlyWhenAllCheckboxSelected").show();
					$(".showOnlyWhenOneCheckboxSelected").show();
				break;
				default:
					$(".showOnlyWhenAllCheckboxSelected").show();
					$(".showOnlyWhenOneCheckboxSelected").hide();
